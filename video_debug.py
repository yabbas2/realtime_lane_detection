...
while True:
    ...
    lines = getAvgLineOfTwoLines(lines)
    ...
    left_pointss = doInverse(left_points, homo)
    right_pointss = doInverse(right_points, homo)
    debug_draw(left_points, magdy)
    debug_draw(right_points, magdy)
    debug_draw(left_points, ipmFrame)
    debug_draw(right_points, ipmFrame)
    debug_draw(left_pointss, normalFrame)
    debug_draw(right_pointss, normalFrame)
    end = time.time()
    print(end-start)
    cv2.imshow('magdyVideo', magdy)
    cv2.imshow('ipmResVideo', ipmFrame)
    cv2.imshow('outputVideo', normalFrame)
stream.release()
